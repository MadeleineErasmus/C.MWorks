...
public partial class CustomerEditViewModel : ObservableObject
{
    ...
    [NotifyPropertyChangedFor(nameof(CanViewStatement))]
    ...
    /// <summary>
    /// Billing history is Admin-only — the Technician app has no Statement
    /// page (or "statement" route) at all, so the button is hidden there.
    /// </summary>
    public bool CanViewStatement => CanDelete && _capabilities.CanViewCustomerFinancials;

    public CustomerEditViewModel(ApiClient api, AppCapabilities capabilities)
    {
        _api = api;
