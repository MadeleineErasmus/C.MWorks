...
public partial class JobCardEditViewModel : ObservableObject
{
    ...
    // Per-company job pricing defaults, refreshed whenever SelectedCompany
    // changes. 500 stays the fallback for the window before a company is
    // picked (and for a company that hasn't configured its own fee), matching
    // the behaviour from when this came from static app settings. Labour has
    // no comparable universal default, so it falls back to 0 — an unconfigured
    // company simply doesn't prefill a labour price.
    private decimal _defaultCallOutFee = 500m;
    private decimal _defaultLabourRate;
    ...
    /// <summary>
    /// Once invoiced, the job card is the source document behind a real
    /// invoice — every editable control on the page is gated on this, and the
    /// API rejects the update too (JobCardsController.Update).
    /// </summary>
    public bool CanEdit => Status != JobCardStatus.Invoiced;

    public decimal Subtotal => Lines.Sum(l => l.LineTotal);

    public JobCardEditViewModel(ApiClient api, AuthState authState)
    {
        _api = api;
    ...
    /// <summary>
    /// "Not on the list?" for the Customer field — creates the customer with
    /// just a name and selects it, which reloads its (empty) items/sites. Any
    /// site for the brand-new customer is then added with the existing Site
    /// inline-add below, so this stays one small step rather than a second
    /// full customer form on top of the job card.
    /// </summary>
    [RelayCommand]
    private async Task AddCustomerAsync()
    {
        ...
        try
        {
            var customer = await _api.CreateCustomerAsync(new Customer { Name = NewCustomerName.Trim() });
            if (customer is not null)
            {
                var existing = Customers.FirstOrDefault(c => c.Id == customer.Id);
                if (existing is null) Customers.Add(customer);
                SelectedCustomer = existing ?? customer;
            }
        }
        catch (Exception ex)
        {
            await Shell.Current.DisplayAlert("Could not add customer", ex.Message, "OK");
            return;
        }
    ...
    /// <summary>
    /// Job pricing defaults are per-company (Company.DefaultCallOutFee /
    /// DefaultLabourRate, editable on the Admin app's Company screen) — pick
    /// them up whenever the billing company changes. A company that hasn't
    /// configured a rate leaves the standing fallback in place.
    /// </summary>
    partial void OnSelectedCompanyChanged(Company? value)
    {
        _defaultCallOutFee = value is { DefaultCallOutFee: > 0 } ? value.DefaultCallOutFee : 500m;
        _defaultLabourRate = value?.DefaultLabourRate ?? 0m;
    }

    /// <summary>
    /// Convenience prefill only: switching to Labour/Call-out drops the
    /// company's configured rate into the price box, but only while it's still
    /// untouched ("0") — a price the user already typed is never overwritten,
    /// and the field stays freely editable afterwards.
    /// </summary>
    partial void OnNewLineKindChanged(LineKind value)
    {
        if (NewLineUnitPrice != "0") return;

        if (value == LineKind.Labour && _defaultLabourRate > 0)
            NewLineUnitPrice = _defaultLabourRate.ToString("0.##");
        else if (value == LineKind.CallOut)
            NewLineUnitPrice = _defaultCallOutFee.ToString("0.##");
    }
    ...
    public async Task LoadAsync()
    {
        ...
        try
        {
            // Pricing defaults are no longer fetched here — they come from
            // whichever Company is selected, via OnSelectedCompanyChanged.
            ...
            else
            {
                ...
                // Whoever is signed in is almost always the technician on the
                // job — a default, not a lock: the field stays editable, and
                // an existing job card's saved technician is never touched.
                Technician ??= _authState.DisplayName;
    ...
    /// <summary>Typing a brand-new category takes precedence — picking from the list afterwards clears it back out, and vice versa, so AddNewCustomerItemAsync always has one unambiguous value to send. Mirrors CustomerEditViewModel.</summary>
    ...
    /// <summary>The categories this customer's existing equipment already uses — the picker's options, so the common case is a pick rather than retyping.</summary>
    private void RebuildCategories()
    {
        var categories = CustomerItems
    ...
    /// SelectedCategory (picked) was used most recently, same as the Customer
    /// screen's Equipment section — and it matters here too, because category
    /// is part of an item's identity server-side.
    /// </summary>
    [RelayCommand]
    private async Task AddNewCustomerItemAsync()
    {
        ...
        try
        {
            var item = await _api.CreateCustomerItemAsync(SelectedCustomer.Id, NewItemName.Trim(), category);
            if (item is not null)
            {
    ...
    private async Task CheckPricingHistoryAsync()
    {
        ...
        try
        {
            // Scoped to the kind currently selected for the line being added,
            // so a Part's price never comes back as a suggestion for Labour.
            var history = await _api.GetPricingHistoryAsync(SelectedCustomer.Id, NewLineDescription, NewLineKind)
    ...
    {
        // The bottom Save button is IsVisible-gated by CanEdit already, but
        // ToolbarItem has no working IsVisible binding in this MAUI version
        // (XamlC rejects it), so the top Save button stays visible even when
        // locked — this guard is what actually stops it from doing anything.
        if (!CanEdit) return;
